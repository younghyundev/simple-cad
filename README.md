...
`서버 저장`은 현재 도면을 브라우저 localStorage 기반 mock 서버 저장소에 저장합니다. 시작 페이지의 `서버 도면` 목록에서 저장된 도면을 다시 열 수 있으며, 이 흐름은 로컬 파일 저장 대상과 별도로 관리됩니다.

`공유`는 서버 저장된 도면에 대해 `?share=` 형식의 공유 링크를 만들고 가능한 경우 클립보드에 복사합니다. 공유 링크로 연 도면은 `읽기 전용 공유 문서`로 표시되며, 이동/삭제/붙여넣기/속성 변경/레이어 변경/주석 상태 변경 같은 편집 동작이 차단됩니다. 다운로드형 내보내기는 도면 원본 상태를 바꾸지 않는 범위에서 사용할 수 있습니다.

우클릭 메뉴의 `주석 추가`로 도면 좌표나 선택 객체에 검토 주석을 남길 수 있습니다. 주석은 캔버스 마커와 오른쪽 `검토` 패널에 표시되며, 편집 가능한 서버 도면에서는 `해결됨` 상태로 전환할 수 있습니다.

현재 서버 저장, 공유 링크, 주석은 실제 인증/권한이 있는 백엔드가 아니라 localStorage mock 모델입니다. 향후 실제 협업 서버로 교체할 수 있도록 `src/cad/collaboration.ts`의 repository 경계로 분리되어 있습니다.
