# Milestone v1.4: Advanced CAD Compatibility Expansion
...
- ✅ **v1.4 — Advanced CAD Compatibility Expansion** — Broader DXF/DWG metadata, style, annotation, block fallback, and layout/reference classification. Shipped 2026-04-26.

## Overview

v1.4 expanded SimpleCAD's real-world CAD compatibility by preserving more DXF/DWG metadata and classifying non-editable CAD references. It focused on layer/style fidelity, annotation and block fallback detail, layout/viewport/reference classification, and a dedicated v1.4 regression fixture.
...
- Preserve CAD information as editable geometry where practical, structured metadata where not editable, and explicit warnings where approximated or unsupported.
- Keep browser-side DXF compatibility incremental and regression-backed instead of attempting full CAD-vendor parity.
- Continue treating DWG as server-converted while sharing the same warning/metadata model.

**Issues Resolved:**

- Layer visibility, lock, linetype, and lineweight metadata are now preserved more consistently.
- TEXT/MTEXT, DIMENSION, ATTRIB/ATTDEF, and BLOCK/INSERT fallback warnings now carry more actionable details.
- Layout, paper space, viewport, IMAGE, XREF, and UNDERLAY information is classified instead of being silently lost or treated as unsupported geometry.
- v1.4 compatibility behavior is locked with a dedicated fixture in `npm run test:cad-fidelity`.

**Issues Deferred:**

- Referenced IMAGE/XREF/UNDERLAY files are classified but not rendered or bundled.
- Full AutoCAD dimension style editing and annotative scale support remain out of scope.
- Real production DWG conversion backend deployment remains deferred.

**Technical Debt Incurred:**

- External references are represented as metadata/warnings only, so future rendering or packaging work will need a reference file management model.
